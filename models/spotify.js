...
function song(req, res, next){
  request("https://api.spotify.com/v1/search?q=work+&type=track", function (error, song) {
    res.song= song
    next()
  })
}
...
function album(req, res, next){
  request("https://api.spotify.com/v1/search?q=no+sound+without+silence&type=album", function (error, record) {
    res.record= record
    next()
  })
}
 module.exports= {artist, song, album}
